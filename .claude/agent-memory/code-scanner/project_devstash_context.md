...
description: Core architectural facts about DevStash relevant to code scanning — auth state, hardcoded user IDs, data access patterns, and what is/isn't built yet
type: project
---
...
- `queryItems` uses `where: object` and `orderBy: object` — weak TypeScript typing.
- `getRecentCollections` fetches ALL items per collection via `include` before doing in-memory type counting.
- `getSidebarCollections` fetches ALL collections for a user with ALL their items for color computation — no limit applied.

## Schema observations
- `Tag` model has no `userId` — tags are global/shared across all users.
- `Item` has no `@@index([userId, createdAt])` composite index.
- `Collection` has no `@@index([userId, isFavorite])` composite index.
- `datasource db` in schema.prisma has no `url = env("DATABASE_URL")` line (omitted from schema, set in prisma.ts directly).

## Dead code
- `src/lib/mock-data.ts` (~400 lines) is not imported anywhere and is dead code.
